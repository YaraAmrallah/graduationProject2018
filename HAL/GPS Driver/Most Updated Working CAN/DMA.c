...
DMA_RetType DMA_StartChannel(uint8_t DMAGroupID,uint32_t SRC, uint32_t DES, uint16_t TransferSize)
{
    ...
    if(TransferSize <= MAX_TRANSFER_SIZE)
    {
        ...
        ((DMA_ADDRESSES*)DMACTLBASE)[Cfg_Ptr->ChannelNo].SRCENDP = (uint32_t)SRC;
        ((DMA_ADDRESSES*)DMACTLBASE)[Cfg_Ptr->ChannelNo].DESENDP = (uint32_t)DES;
        //assigning transfer size to the CTRL Word
        CTRL_Words[Cfg_Ptr->DMA_ID] |= ((TransferSize - TRANSFER_SIZE_DEC) <<  TRANSFER_SIZE_OFFSET);
        ((DMA_ADDRESSES*)DMACTLBASE)[Cfg_Ptr->ChannelNo].CHCTLP |= CTRL_Words[Cfg_Ptr->DMA_ID];
